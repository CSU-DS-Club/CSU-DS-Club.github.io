...
1. for i=1 to m
   ...
   计算 $$x^{(i)}$$ 距离最近的聚类簇的中心，将其作为 $$x^{(i)}$$ 的类别，即 $$y^{(i)}=\arg\min_k{\mid \mid x^{(i)}-\mu_k\mid \mid ^2}$$

2. for k=1 to K

   更新聚类簇的中心，用所有属于第 k 个簇的样本的均值去更新 $$\mu_k$$ ，即 $$\mu_k=avg(x^{(i)}\mid y^{(i)}=k)$$
...
$$
SSE=\sum_{i-1}^{k}\sum_{p\in C_i}\mid p-\mu_i\mid ^2
$$
...
$$
C_j=\arg\min_{C_k}\frac{1}{n}\sum_{p\in C_k}\mid p-x_i\mid ^2
$$
其中 p 是某个簇 $$C_k$$ 中的样本。
